  ...
  if (!user) {
    return (
      <div className="flex-grow max-w-xl mx-auto px-4 py-24 text-center space-y-6 bg-background text-foreground">
        ...
        <h1 className="font-serif font-black text-2.5xl text-navy">Amore Cakes Profile Desk</h1>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 bg-background text-foreground animate-fade-in">
      ...
      <div className="bg-cream border border-border-color p-6 md:p-8 rounded-[32px] flex flex-col md:flex-row justify-between items-center gap-6 shadow-sm">
        <div className="flex gap-4 items-center flex-col md:flex-row text-center md:text-left">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-serif font-black text-navy">{user.name}</h1>
            <p className="text-xs text-navy/55 font-black flex items-center justify-center md:justify-start gap-1"><Mail className="w-4 h-4 text-navy/40" /> {user.email}</p>
            <p className="text-[10px] bg-orange/15 text-orange border border-orange/20 rounded-full py-0.5 px-2.5 font-black uppercase tracking-wider inline-block">VIP Shopper Profile</p>
          </div>
        </div>

        <button 
          onClick={logout}
          className="border border-red-200 hover:bg-red-50 text-red-500 font-black py-2.5 px-6 rounded-2xl text-xs transition-colors"
        >
          Sign Out of Profile
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-3 space-y-3">
          <div className="bg-card-bg border border-border-color rounded-[28px] p-4 flex flex-col gap-1.5 shadow-md font-black text-xs text-navy">
            <button 
        ...
        <div className="lg:col-span-9 space-y-6">
          ...
          {activeTab === 'profile' && (
            <div className="bg-card-bg border border-border-color rounded-[28px] p-6 space-y-4 shadow-md text-navy animate-fade-in">
              <h2 className="font-serif font-black text-lg border-b border-border-color pb-2.5">Shopper Profile Settings</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs font-semibold">
                <div className="space-y-1.5">
                  <span className="text-[10px] uppercase font-bold text-navy/50">Full Name</span>
                  <input type="text" defaultValue={user.name} disabled className="w-full p-2.5 bg-background border border-border-color rounded-xl outline-none text-navy font-bold" />
                </div>
                <div className="space-y-1.5">
                  <span className="text-[10px] uppercase font-bold text-navy/50">Email Credentials</span>
                  <input type="email" defaultValue={user.email} disabled className="w-full p-2.5 bg-background border border-border-color rounded-xl outline-none text-navy font-bold" />
                </div>
              </div>
              <div className="text-[10px] text-navy/40 pt-2 border-t border-border-color flex items-center gap-1.5 font-bold">
            ...
            <div className="space-y-6 animate-fade-in">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {user.addresses.map((addr) => (
                    <div key={addr._id} className="bg-card-bg border border-border-color p-5 rounded-2xl relative shadow-sm text-navy">
                      ...
                      <div className="flex gap-1.5 items-center text-xs font-serif font-black text-navy">
                        <span className="text-sm">{addr.name}</span>
                        {addr.isDefault && <span className="bg-orange/15 text-orange border border-orange/20 uppercase text-[9px] font-sans px-1.5 py-0.5 rounded font-black">Default</span>}
                      </div>
                      <p className="text-xs text-navy/75 font-semibold mt-2 leading-relaxed">{addr.street}, {addr.city}, {addr.state} - {addr.zipCode}</p>
              ...
              {!showAddressForm ? (
                <button 
                  ...
                  className="border border-dashed border-border-color bg-card-bg hover:border-orange rounded-2xl p-4 w-full text-center text-xs font-black text-navy flex items-center justify-center gap-1.5 transition-all"
                >
                ...
                <form onSubmit={handleAddNewAddress} className="bg-card-bg border border-border-color p-6 rounded-3xl space-y-4 shadow-md text-navy">
                  <div className="flex justify-between items-center pb-2 border-b border-border-color">
                    <h3 className="font-serif font-black text-navy text-sm">Add New Saved Address</h3>
                    <button type="button" onClick={() => setShowAddressForm(false)} className="text-xs text-red-500 font-bold hover:underline">Cancel</button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input 
                      ...
                      className="text-xs font-semibold p-2.5 bg-background border border-border-color rounded-xl outline-none text-navy"
                    />
                    <input 
              ...
              <div className="lg:col-span-5 space-y-4 max-h-[500px] overflow-y-auto no-scrollbar pr-1">
                {orders.length === 0 ? (
                  <div className="bg-card-bg border border-border-color rounded-2xl p-8 text-center space-y-2 text-navy">
                    <p className="text-xs text-navy/40 italic font-semibold">No checkout histories found.</p>
                    <Link href="/category/all" className="text-xs font-black text-orange hover:underline block">Explore Confectioneries</Link>
                  </div>
                ) : (
                  orders.map(order => (
                    <div 
                    ...
                    >
                      <div className="flex justify-between items-center text-xs font-black">
                        <span className="font-serif text-orange font-black">{order.invoiceNumber}</span>
                        <span className={`text-[9px] uppercase px-2 py-0.5 rounded-full font-black ${order.status === 'Delivered' ? 'bg-success/10 border border-success/20 text-success' : order.status === 'Cancelled' ? 'bg-red-50 border border-red-100 text-red-600' : 'bg-orange/15 text-orange border border-orange/20'}`}>{order.status}</span>
                      </div>
                      <div className="text-[10px] text-navy/50 font-semibold">
              ...
              {trackingOrder && (
                <div className="lg:col-span-7 bg-card-bg border border-border-color rounded-[28px] p-6 space-y-6 shadow-md text-navy">
                  <div className="flex justify-between items-start border-b border-border-color pb-3">
                    <div>
                      <h3 className="font-serif font-black text-navy text-base">Shipment Tracking Desk</h3>
                      <p className="text-[10px] text-navy/50 font-black uppercase mt-0.5">Details for: {trackingOrder.invoiceNumber}</p>
                    </div>
                  ...
                  <div className="space-y-6">
                    <p className="text-[10px] font-black uppercase tracking-widest text-navy/50">Delivery Timeline Status</p>
                    
                    <div className="relative pl-8 space-y-6 text-xs font-black text-navy">
                      {/* Spine bar */}
                    ...
                    <div className="pt-4 border-t border-border-color flex justify-between items-center text-xs font-black">
          ...
          {activeTab === 'wishlist' && (
            <div className="bg-card-bg border border-border-color rounded-[28px] p-6 space-y-4 shadow-md text-navy animate-fade-in">
              <h2 className="font-serif font-black text-lg border-b border-border-color pb-2.5">Your Wishlisted Confectioneries</h2>
              
              {wishlistProducts.length === 0 ? (
                <div className="p-8 text-center space-y-2">
                  <p className="text-xs text-navy/40 italic font-semibold">Your wishlist is currently clear.</p>
                  <Link href="/category/all" className="text-xs font-black text-orange hover:underline block uppercase">Explore Catalog</Link>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {wishlistProducts.map((prod) => (
                    <div key={prod._id} className="border border-border-color rounded-2xl p-3 flex gap-3 items-center justify-between hover:border-orange transition-colors bg-cream">
                      ...
                      <div className="truncate flex-1 font-black text-xs">
      ...
      {showCancelModal && trackingOrder && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <form onSubmit={handleCancelOrderSubmit} className="bg-card-bg border border-border-color rounded-3xl p-6 md:p-8 max-w-md w-full space-y-4 shadow-2xl relative text-navy">
            ...
            <div className="flex gap-2.5 items-center text-red-500 font-serif font-black text-lg">
              <AlertTriangle className="w-6 h-6 animate-pulse" /> Confirm Cancellation
            </div>
            <p className="text-xs text-navy/75 font-semibold leading-relaxed">
            ...
            <div className="space-y-1">
              <label className="text-[10px] uppercase font-black text-navy/55 tracking-wider">Reason for Cancellation</label>
              <textarea 
                required rows={3} placeholder="Please tell our chef why you need to cancel..."
                value={cancelReason} onChange={(e) => setCancelReason(e.target.value)}
                className="w-full text-xs font-semibold p-3 border border-border-color bg-background text-navy rounded-xl outline-none focus:ring-1 focus:ring-orange"
              />
            </div>
            <div className="flex gap-3 pt-2 font-black text-xs">
    ...
    <div className="relative pl-1">
      ...
      <div className={`${active ? 'text-navy' : 'text-navy/40'}`}>
        <div className="flex justify-between items-baseline flex-wrap gap-2 text-xs">
          <h4 className="font-serif font-black text-sm">{title}</h4>
          {timestamp && <span className="text-[9px] font-black text-navy/45">{new Date(timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>}
        </div>
        <p className={`text-xs mt-1 leading-normal ${active ? 'text-navy/65' : 'text-navy/30'} font-semibold`}>{desc}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <Suspense fallback={
      <div className="flex-grow flex items-center justify-center py-24 bg-background text-navy gap-3">
        ...
        <span className="font-sans font-black animate-pulse uppercase tracking-wider text-xs">Cooking Profile...</span>
      </div>
    }>
      <DashboardContent />
    </Suspense>
