  ...
  if (loading) {
    return (
      <div className="flex-grow flex items-center justify-center py-24 bg-background text-navy gap-3">
        ...
        <span className="font-sans font-black animate-pulse uppercase tracking-wider text-xs">Sifting products...</span>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 bg-background text-foreground">
      ...
      <div className="bg-cream border border-border-color p-8 md:p-12 rounded-[32px] text-center space-y-4 shadow-sm relative overflow-hidden">
        ...
        <span className="text-[10px] font-black uppercase tracking-widest text-orange bg-orange/15 px-3 py-1 rounded-full border border-orange/20">Artisanal Bakery</span>
        <h1 className="text-3xl md:text-5xl font-serif font-black text-navy leading-none">{title}</h1>
        <p className="text-navy/60 text-xs md:text-sm max-w-2xl mx-auto leading-relaxed font-semibold">{description}</p>
      </div>

      {/* Mobile Filter Toggle Button */}
      <button 
        onClick={() => setMobileFiltersOpen(!mobileFiltersOpen)}
        className="lg:hidden w-full bg-orange hover:bg-orange-hover text-white font-black py-3 px-6 rounded-2xl text-xs flex items-center justify-center gap-2 shadow-md transition-all active:scale-98"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className={`lg:col-span-1 space-y-6 ${mobileFiltersOpen ? 'block' : 'hidden lg:block'}`}>
          <div className="bg-card-bg border border-border-color rounded-[28px] p-6 space-y-6 shadow-md">
            ...
            <div className="flex justify-between items-center border-b border-border-color pb-3">
              <span className="font-serif font-black text-base text-navy flex items-center gap-1.5">
                <Sliders className="w-4 h-4 text-orange" /> Catalog Filters
              </span>
              <button 
                onClick={handleClearFilters}
                className="text-[10px] font-black text-red-500 uppercase flex items-center gap-1 hover:text-red-700"
              >
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-navy/50">Search Within</label>
              <div className="flex items-center border border-border-color bg-cream/20 rounded-xl py-1.5 px-3">
                ...
                <input 
                  ...
                  className="w-full bg-transparent border-none outline-none text-xs text-navy font-bold"
                />
              </div>
            </div>
            ...
            {/* Price Max slider */}
            <div className="space-y-2">
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-navy/50">
                <span>Maximum Price</span>
                <span className="text-orange font-black text-xs">₹{priceMax}</span>
              </div>
              <input 
                type="range" 
                min={150} 
            ...
            <div className="space-y-2.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-navy/50">Minimum Rating</label>
              <div className="flex flex-col gap-1.5">
                {[0, 4.5, 4.2, 4.0].map((rate) => (
                  <button 
                    key={rate}
                    onClick={() => setMinRating(rate)}
                    className={`flex items-center justify-between text-xs font-black px-4 py-2 rounded-xl border transition-all text-left ${minRating === rate ? 'bg-orange text-white border-orange shadow-sm animate-pulse-short' : 'border-border-color hover:border-orange bg-background text-navy'}`}
                  >
            ...
            <div className="space-y-2.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-navy/50 flex items-center gap-1">
                <ArrowUpDown className="w-3.5 h-3.5 text-orange" /> Sort Results
              </label>
              <select 
                value={sortOption}
                onChange={(e) => setSortOption(e.target.value)}
                className="w-full text-xs font-bold border border-border-color p-2.5 rounded-xl bg-card-bg text-navy outline-none focus:ring-1 focus:ring-orange cursor-pointer"
              >
            ...
            <label className="flex items-center gap-2 text-xs font-black text-navy cursor-pointer select-none border-t border-border-color pt-4">
              <input 
                type="checkbox" 
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {/* Header row stats */}
          <div className="flex justify-between items-center bg-card-bg border border-border-color py-3.5 px-5 rounded-2xl text-xs font-black text-navy/60 shadow-sm">
            <span>Showing {filtered.length} elegant results matching selections</span>
            <span className="hidden sm:inline text-orange uppercase font-black tracking-wider">Free Deliveries above ₹499!</span>
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="bg-card-bg border border-border-color rounded-[32px] p-16 text-center space-y-4 shadow-sm">
              ...
              <h3 className="font-serif font-black text-navy text-xl">No Confectioneries Found</h3>
              <p className="text-navy/50 text-xs md:text-sm max-w-sm mx-auto font-semibold">We couldn&apos;t locate records matching selections. Try broadening search queries or clearing filters.</p>
              <button 
                onClick={handleClearFilters}
                className="bg-orange hover:bg-orange-hover text-white font-black py-2.5 px-6 rounded-full text-xs transition-all shadow-md hover:scale-102"
              >
                Reset Filters
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filtered.map((prod) => {
                ...
                return (
                  <div 
                    key={prod._id}
                    className="bg-card-bg border border-border-color rounded-3xl p-4 flex flex-col justify-between group hover:border-orange amore-card shadow-sm transition-all duration-300 relative overflow-hidden"
                  >
                    {/* Wishlist Button */}
                    <button 
                    ...
                    <Link href={`/product/${prod.slug}`} className="space-y-3 cursor-pointer block">
                      <div className="rounded-2xl overflow-hidden aspect-video border border-border-color relative">
                        ...
                        />
                        {prod.isPremium && (
                          <span className="absolute bottom-2 left-2 bg-orange text-white font-black text-[9px] px-2 py-0.5 rounded-full border border-orange/30">Premium</span>
                        )}
                      </div>

                      <div className="space-y-1">
                        <div className="flex items-center gap-1 text-[10px] text-navy/60 font-black">
                          <Star className="w-3.5 h-3.5 fill-gold text-gold" />
                          <span>{prod.rating} ({prod.reviewsCount} orders)</span>
                        </div>
                        <h3 className="font-serif font-black text-navy text-sm leading-tight truncate group-hover:text-orange transition-colors">{prod.name}</h3>
                        <p className="text-navy/50 text-[10px] truncate leading-normal font-semibold">{prod.description}</p>
                      </div>
                    </Link>
                    ...
                    <div className="mt-3 pt-3 border-t border-border-color flex items-center justify-between">
                      <div>
                        ...
                        {prod.discountPrice && (
                          <span className="text-navy/35 line-through text-xs ml-1 font-bold">₹{prod.price}</span>
                        )}
                      </div>
...
export default function CategoryPage({ params }) {
  ...
  return (
    <Suspense fallback={
      <div className="flex-grow flex items-center justify-center py-24 bg-background text-navy gap-3">
        <div className="w-12 h-12 border-t-2 border-b-2 border-orange rounded-full animate-spin"></div>
        <span className="font-sans font-black animate-pulse uppercase tracking-wider text-xs">Cooking Catalog...</span>
      </div>
    }>
